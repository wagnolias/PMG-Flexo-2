...
const PostCard: React.FC<{ post: BlogPost }> = ({ post }) => (
  <motion.a
    ...
    className="group flex flex-col rounded-3xl overflow-hidden bg-white border border-slate-200 hover:border-pmg-magenta/40 hover:shadow-xl transition-all duration-300"
  >
    ...
    <div className="p-6 flex-1 flex flex-col">
      ...
      <h3 className="text-lg font-bold text-pmg-dark group-hover:text-pmg-magenta transition-colors mb-2 leading-snug">
        {post.title}
      </h3>
      <p className="text-sm text-slate-600 font-light leading-relaxed flex-1">{post.excerpt}</p>
      <span className="mt-4 inline-flex items-center gap-1.5 text-xs font-bold text-pmg-magenta">
    ...
    <section id="blog" className="py-20 sm:py-28 bg-slate-50 text-pmg-dark relative">
      <div className="max-w-7xl mx-auto px-6">
        <div className="mb-12 max-w-2xl">
          <div className="inline-flex items-center gap-2.5 px-3.5 py-1.5 rounded-full bg-pmg-magenta/10 border border-pmg-magenta/20 text-xs font-semibold text-pmg-magenta uppercase tracking-wider mb-4">
            <Rss size={13} />
            <span>Blog PMG Flexo</span>
          </div>

          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-pmg-navy tracking-tight">
            Conteúdo técnico de <span className="text-pmg-magenta">clicheria flexográfica</span>.
          </h2>
          <p className="text-slate-600 text-sm sm:text-base font-light mt-3 leading-relaxed">
            Artigos técnicos, atualizações de tecnologia e bastidores da produção — direto de quem
            opera a gravação todos os dias.
          </p>
        </div>
...
const BlogPostView: React.FC<{ post: BlogPost }> = ({ post }) => (
  <section className="py-20 sm:py-28 bg-slate-50 text-pmg-dark">
    <div className="max-w-3xl mx-auto px-6">
      <a
        href="#blog"
        className="inline-flex items-center gap-1.5 text-xs font-bold text-pmg-magenta mb-8 hover:underline"
      >
        <ArrowLeft size={14} /> Voltar para o blog
      </a>

      <span className="text-xs font-bold text-pmg-magenta uppercase tracking-widest block mb-3">
        {post.category}
      </span>
